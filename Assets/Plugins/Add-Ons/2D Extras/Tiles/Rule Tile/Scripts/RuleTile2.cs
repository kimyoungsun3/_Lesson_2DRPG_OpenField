...
namespace UnityEngine
{
	...
	public class RuleTile2 : TileBase
	{
		...
		[HideInInspector] public List<TilingRule2> m_TilingRules;
		...
		public override void GetTileData(Vector3Int position, ITilemap tileMap, ref TileData tileData)
		{
			...
			foreach (TilingRule2 rule in m_TilingRules) 
			{
				Matrix4x4 transform = Matrix4x4.identity;
				if (RuleMatches (rule, position, tileMap, ref transform)) {
					switch (rule.m_Output) {
					case TilingRule2.OutputSprite.Single:
					case TilingRule2.OutputSprite.Animation:
						tileData.sprite = rule.m_Sprites [0];
						break;
					case TilingRule2.OutputSprite.Random:
						int index = Mathf.Clamp (Mathf.FloorToInt (GetPerlinValue (position, rule.m_PerlinScale, 100000f) * rule.m_Sprites.Length), 0, rule.m_Sprites.Length - 1);
						tileData.sprite = rule.m_Sprites [index];
						if (rule.m_RuleTransform != TilingRule2.Transform.Fixed) {
							transform = ApplyRandomTransform (rule.m_RuleTransform, transform, rule.m_PerlinScale, position);
						}
						break;
					}
		...
		public override bool GetTileAnimationData(Vector3Int position, ITilemap tilemap, ref TileAnimationData tileAnimationData){
			foreach (TilingRule2 rule in m_TilingRules) {
				Matrix4x4 transform = Matrix4x4.identity;
				if (RuleMatches (rule, position, tilemap, ref transform) && rule.m_Output == TilingRule2.OutputSprite.Animation) {
		...
		private static Matrix4x4 ApplyRandomTransform(TilingRule2.Transform type, Matrix4x4 original, float perlinScale, Vector3Int position){
			float perlin = GetPerlinValue (position, perlinScale, 200000f);
			switch (type) {
			case TilingRule2.Transform.MirrorX:
				return original * Matrix4x4.TRS (Vector3.zero, Quaternion.identity, new Vector3 (perlin < 0.5f ? 1f : -1f, 1f, 1f));
			case TilingRule2.Transform.MirrorY:
				return original * Matrix4x4.TRS (Vector3.zero, Quaternion.identity, new Vector3 (1f, perlin < 0.5f ? 1f : -1f, 1f));
			case TilingRule2.Transform.Rotated:
		...
		public bool RuleMatches(TilingRule2 rule, Vector3Int position, ITilemap tilemap, bool mirrorX, bool mirrorY){
			for (int y = -1; y <= 1; y++) {
				for (int x = -1; x <= 1; x++) {
					//Out line
					if (!(x == 0 && y == 0)) {
